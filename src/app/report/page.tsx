    ...
    try {
      ...
      try {
        ...
        if (parsedResult.wasteType && parsedResult.quantity && parsedResult.confidence !== undefined) {
          ...
          // Update form fields with the AI results
          setNewReport(prev => ({
            ...prev,
          ...
          console.error('Incomplete verification result:', parsedResult);
          setVerificationStatus('failure');
          toast.error('Verification failed: Incomplete AI response');
        }
      } catch (error) {
        console.error('Failed to parse JSON response:', text, error);
        setVerificationStatus('failure');
        toast.error('Verification failed: Could not parse AI response');
      }
    } catch (error) {
      console.error('Error verifying waste:', error);
      setVerificationStatus('failure');
      toast.error('Verification failed: API error');
    }
  }
